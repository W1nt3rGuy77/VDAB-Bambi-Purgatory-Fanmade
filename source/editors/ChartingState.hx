...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		tab_group_song.add(new FlxText(noteSkinInputText.x, noteSkinInputText.y - 15, 0, 'Opponent Note Texture:'));
	...
	function loadJson(song:String):Void
	{
		...
		if (song.toLowerCase() == 'unfairness' || song.toLowerCase() == 'cheating' || song.toLowerCase() == 'disruption' || song.toLowerCase() == 'opposition' || song.toLowerCase() == 'disposition')
			{
				FlxG.switchState(new SusState()); //YOU THINK YOU ARE SO CLEVER DON'T YOU? HAHA FUCK YOU
			}
		if (song.toLowerCase() == 'screwed') {
